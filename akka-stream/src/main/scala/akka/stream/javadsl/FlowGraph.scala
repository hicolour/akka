...
object Zip {
  ...
  /**
   * Create a new `ZipWith` named "zip" vertex with the specified input types and zipping-function
  ...
  def create[A, B]: ZipWith[A, B, A Pair B] =
    ZipWith.create("zip", _toPair.asInstanceOf[Function2[A, B, A Pair B]])
