...
object Zip {
  // FIXME doc
  def apply[A, B]: ZipWith[A, B, (A, B)] = new ZipWith(_name, _toTuple.asInstanceOf[(A, B) ⇒ (A, B)])
  // FIXME doc
  def apply[A, B](name: String): ZipWith[A, B, (A, B)] = new ZipWith(Some(name), _toTuple.asInstanceOf[(A, B) ⇒ (A, B)])
  private[this] final val _toTuple: (Any, Any) ⇒ (Any, Any) = (a, b) ⇒ (a, b)
  private[this] final val _name: Option[String] = Some("zip")
}
...
object ZipWith {
  ...
  // FIXME should this be visible?
  class Left[A, B, C] private[akka] (private[akka] val vertex: ZipWith[A, B, C]) extends JunctionInPort[A] {
    type NextT = C
    override private[akka] def port = 0
    override private[akka] def next = vertex.out
  }
  // FIXME should this be visible?
  class Right[A, B, C] private[akka] (private[akka] val vertex: ZipWith[A, B, C]) extends JunctionInPort[B] {
    type NextT = C
    override private[akka] def port = 1
    override private[akka] def next = vertex.out
  }
  // FIXME should this be visible?
  class Out[A, B, C] private[akka] (private[akka] val vertex: ZipWith[A, B, C]) extends JunctionOutPort[C]
}
